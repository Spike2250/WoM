...
def tpl_stamp(d, i):
    ...
    rehab_diagnosis_1 = f"Cтруктура (s):\n{d['домены_структуры']}\n"\
                        f"Функции (b):\n{d['домены_функций']}\n"\
                        f"Активность и участие (d):\n"\
                        f"{d['домены_активности_и_участия']}\n"\
                        f"Окружающая среда (e):\n"\
                        f"{d['домены_окружающей_среды']}"
    rehab_diagnosis_2 = f"Cтруктура (s):\n{d['домены_структуры_вып']}\n"\
                        f"Функции (b):\n{d['домены_функций_вып']}\n"\
                        f"Активность и участие (d):\n"\
                        f"{d['домены_активности_и_участия_вып']}\n"\
                        f"Окружающая среда (e):\n"\
                        f"{d['домены_окружающей_среды_вып']}"
    if 'icf_psy' in d:
        rehab_diagnosis_1 += f"\nДомены психолога:\n"\
                             f"{d[f'icf_psy_text_adm_no_header']}"
        rehab_diagnosis_2 += f"\nДомены психолога:\n"\
                             f"{d[f'icf_psy_text_dis_no_header']}"
    if 'icf_logo' in d:
        rehab_diagnosis_1 += f"\nДомены логопеда:\n"\
                             f"{d[f'icf_logo_text_adm_no_header']}"
        rehab_diagnosis_2 += f"\nДомены логопеда:\n"\
                             f"{d[f'icf_logo_text_dis_no_header']}"
